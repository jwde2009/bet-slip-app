...
function parseDraftKingsSoccerVisibleMarkets(block, event, away, home, context) {
  const rows = [];
  ...
  function sameTeam(value = "", team = "") {
    return normalizeTeamName(value).toLowerCase() === normalizeTeamName(team).toLowerCase();
  }

  function normalizeDkSoccerSelection(value = "") {
  ...
  function isSoccerVisibleHeader(value = "") {
    const text = normalizeLine(value);

    return (
      /^(Moneyline|Tie No Bet|Total Goals|Spread|Double Chance|Both Teams to Score|Draw No Bet|Total Corners|Total Team Corners|Team Clean Sheet|Correct Score|Goalscorer|Goals \+ Assists|Player Shots|Player Shots on Target|Player Assists|Total Team Goals|Moneyline - Halves|Halftime or Fulltime|Asian Corners|Total Corners \(3-Way\)|Corners Winner \(3-Way\)|Last Corner|Race to X Corners|DraftKings Inc\.)$/i.test(text) ||
      /^Both Teams to Score\s*\//i.test(text) ||
      /^Both Teams to Score No Draw$/i.test(text) ||
      /^Total Corners\s*-\s*/i.test(text) ||
      /^Total Team Corners\s*-\s*/i.test(text) ||
      /^Correct Score/i.test(text) ||
      isHardStopLine(text)
    );
  }

  function sectionEnd(startIndex, currentHeaderPattern = null) {
    for (let i = startIndex; i < block.length; i += 1) {
      const line = normalizeLine(block[i]);

      // DK commonly repeats the active market title once. Do not let the
      // duplicate title end the section before the odds.
      if (
        currentHeaderPattern &&
        i <= startIndex + 2 &&
        currentHeaderPattern.test(line)
      ) {
        continue;
      }

      if (isSoccerVisibleHeader(line)) return i;
    }

    return block.length;
  }

  function findAmericanOddsAfter(startIndex, endIndex) {
    for (let i = startIndex; i < endIndex; i += 1) {
      const odds = parseAmericanOdds(block[i]);
      if (odds !== null) return { odds, index: i };
    }

    return { odds: null, index: -1 };
  }

  function findNumberAfter(startIndex, endIndex) {
    for (let i = startIndex; i < endIndex; i += 1) {
      const parsed = parseTotalNumber(block[i]);
      if (parsed !== null) return { value: parsed, index: i };
    }

    return { value: null, index: -1 };
  }

  function findHeaderIndexes(pattern) {
    const indexes = [];

    for (let i = 0; i < block.length; i += 1) {
      if (pattern.test(normalizeLine(block[i]))) indexes.push(i);
    }
  ...
  // Moneyline / 3-way result.
  for (const moneylineIdx of findHeaderIndexes(/^Moneyline$/i)) {
    const end = sectionEnd(moneylineIdx + 1, /^Moneyline$/i);
    let awayOdds = null;
    let drawOdds = null;
    let homeOdds = null;

    for (let i = moneylineIdx + 1; i < end - 1; i += 1) {
      const label = normalizeLine(block[i]);
      const odds = parseAmericanOdds(block[i + 1]);
      if (odds === null) continue;

      if (sameTeam(label, awayNorm)) awayOdds = odds;
      if (/^Draw$/i.test(label)) drawOdds = odds;
      if (sameTeam(label, homeNorm)) homeOdds = odds;
    }

    if (awayOdds !== null && drawOdds !== null && homeOdds !== null) {
      pushRow(makeRow({ event, selection: away, marketType: "moneyline_3way", lineValue: null, oddsAmerican: awayOdds, context }));
      pushRow(makeRow({ event, selection: "Draw", marketType: "moneyline_3way", lineValue: null, oddsAmerican: drawOdds, context }));
      pushRow(makeRow({ event, selection: home, marketType: "moneyline_3way", lineValue: null, oddsAmerican: homeOdds, context }));
    }
  }

  // Total Goals. Prefer the highlighted top O/U pair.
  for (const totalGoalsIdx of findHeaderIndexes(/^Total Goals$/i)) {
    const end = sectionEnd(totalGoalsIdx + 1, /^Total Goals$/i);
    let parsed = false;

    for (let i = totalGoalsIdx + 1; i < end; i += 1) {
      if (!/^OVER$/i.test(normalizeLine(block[i]))) continue;

      const overLine = findNumberAfter(i + 1, Math.min(end, i + 8));
      if (overLine.value === null) continue;

      const overOdds = findAmericanOddsAfter(overLine.index + 1, Math.min(end, overLine.index + 6));
      if (overOdds.odds === null) continue;

      let underIndex = -1;
      for (let j = overOdds.index + 1; j < Math.min(end, overOdds.index + 12); j += 1) {
        if (/^UNDER$/i.test(normalizeLine(block[j]))) {
          underIndex = j;
          break;
        }
      }
      ...
      const underLine = findNumberAfter(underIndex + 1, Math.min(end, underIndex + 8));
      if (underLine.value === null || Math.abs(underLine.value - overLine.value) > 0.0001) continue;

      const underOdds = findAmericanOddsAfter(underLine.index + 1, Math.min(end, underLine.index + 6));
      if (underOdds.odds === null) continue;

      pushRow(makeRow({ event, selection: "Over", marketType: "total", lineValue: overLine.value, oddsAmerican: overOdds.odds, context }));
      pushRow(makeRow({ event, selection: "Under", marketType: "total", lineValue: underLine.value, oddsAmerican: underOdds.odds, context }));
      parsed = true;
      break;
    }
  ...
  // Spread / handicap if DK exposes the spread table.
  for (const spreadIdx of findHeaderIndexes(/^Spread$/i)) {
    const end = sectionEnd(spreadIdx + 1, /^Spread$/i);
    let awayLine = null;
    let awayOdds = null;
    let homeLine = null;
    let homeOdds = null;

    for (let i = spreadIdx + 1; i < end - 2; i += 1) {
      const label = normalizeLine(block[i]);

      if (sameTeam(label, awayNorm)) {
        const lineInfo = findNumberAfter(i + 1, Math.min(end, i + 8));
        const oddsInfo = lineInfo.value === null ? { odds: null } : findAmericanOddsAfter(lineInfo.index + 1, Math.min(end, lineInfo.index + 6));
        if (lineInfo.value !== null && oddsInfo.odds !== null) {
          awayLine = lineInfo.value;
          awayOdds = oddsInfo.odds;
        }
      }

      if (sameTeam(label, homeNorm)) {
        const lineInfo = findNumberAfter(i + 1, Math.min(end, i + 8));
        const oddsInfo = lineInfo.value === null ? { odds: null } : findAmericanOddsAfter(lineInfo.index + 1, Math.min(end, lineInfo.index + 6));
        if (lineInfo.value !== null && oddsInfo.odds !== null) {
          homeLine = lineInfo.value;
          homeOdds = oddsInfo.odds;
        }
      }
    }

    if (awayLine !== null && awayOdds !== null && homeLine !== null && homeOdds !== null) {
      pushRow(makeRow({ event, selection: away, marketType: "spread", lineValue: awayLine, oddsAmerican: awayOdds, context }));
      pushRow(makeRow({ event, selection: home, marketType: "spread", lineValue: homeLine, oddsAmerican: homeOdds, context }));
      break;
    }
  }

  // Both Teams To Score — full match only.
  for (const bttsIdx of findHeaderIndexes(/^Both Teams to Score$/i)) {
    const end = sectionEnd(bttsIdx + 1, /^Both Teams to Score$/i);
    let yesOdds = null;
    let noOdds = null;
    ...
    for (let i = bttsIdx + 1; i < end - 1; i += 1) {
      ...
      if (odds === null) continue;
      if (/^Yes$/i.test(label)) yesOdds = odds;
      if (/^No$/i.test(label)) noOdds = odds;
    }

    if (yesOdds !== null && noOdds !== null) {
      pushRow(makeRow({ event, selection: "Yes", marketType: "both_teams_to_score", lineValue: null, oddsAmerican: yesOdds, context }));
      pushRow(makeRow({ event, selection: "No", marketType: "both_teams_to_score", lineValue: null, oddsAmerican: noOdds, context }));
      break;
    }
  }

  // Double Chance — full match only.
  for (const doubleChanceIdx of findHeaderIndexes(/^Double Chance$/i)) {
    const end = sectionEnd(doubleChanceIdx + 1, /^Double Chance$/i);
    let parsedCount = 0;

    for (let i = doubleChanceIdx + 1; i < end - 1; i += 1) {
      const label = normalizeDkSoccerSelection(block[i]);
      if (!/\bOr\b/i.test(label)) continue;

      const odds = parseAmericanOdds(block[i + 1]);
      if (odds === null) continue;

      pushRow(makeRow({ event, selection: label, marketType: "double_chance", lineValue: null, oddsAmerican: odds, context }));
      parsedCount += 1;
      i += 1;
    }

    if (parsedCount >= 2) break;
  }

  // Total Corners.
  for (const totalCornersIdx of findHeaderIndexes(/^Total Corners$/i)) {
    const end = sectionEnd(totalCornersIdx + 1, /^Total Corners$/i);
    let parsed = false;

    for (let i = totalCornersIdx + 1; i < end; i += 1) {
      if (!/^OVER$/i.test(normalizeLine(block[i])) && !/^Over$/i.test(normalizeLine(block[i]))) continue;
      ...
      let underIndex = -1;
      for (let j = overOdds.index + 1; j < Math.min(end, overOdds.index + 12); j += 1) {
        if (/^UNDER$/i.test(normalizeLine(block[j])) || /^Under$/i.test(normalizeLine(block[j]))) {
          underIndex = j;
          break;
        }
      }
      ...
      const underLine = findNumberAfter(underIndex + 1, Math.min(end, underIndex + 8));
      const underOdds = underLine.value === null ? { odds: null } : findAmericanOddsAfter(underLine.index + 1, Math.min(end, underLine.index + 6));

      if (
        underLine.value !== null &&
        Math.abs(underLine.value - overLine.value) < 0.0001 &&
        underOdds.odds !== null
      ) {
        pushRow(makeRow({ event, selection: "Over", marketType: "corner_total", lineValue: overLine.value, oddsAmerican: overOdds.odds, context }));
        pushRow(makeRow({ event, selection: "Under", marketType: "corner_total", lineValue: underLine.value, oddsAmerican: underOdds.odds, context }));
        parsed = true;
        break;
      }
    }
  ...
  // Team Total Corners.
  for (const teamCornersIdx of findHeaderIndexes(/^Total Team Corners$/i)) {
    const end = sectionEnd(teamCornersIdx + 1, /^Total Team Corners$/i);

    for (let i = teamCornersIdx + 1; i < end - 5; i += 1) {
      const header = normalizeLine(block[i]);
      const teamMatch = header.match(/^(.+?):\s*Team Total Corners$/i);
      if (!teamMatch) continue;

      const team = normalizeTeamName(teamMatch[1]);
      const overLabel = normalizeLine(block[i + 1]);
      const overLine = parseTotalNumber(block[i + 2]);
      const overOdds = parseAmericanOdds(block[i + 3]);
      const underLabel = normalizeLine(block[i + 4]);
      const underLine = parseTotalNumber(block[i + 5]);
      const underOdds = parseAmericanOdds(block[i + 6]);

      if (
        /^Over$/i.test(overLabel) &&
        /^Under$/i.test(underLabel) &&
        overLine !== null &&
        underLine !== null &&
        Math.abs(overLine - underLine) < 0.0001 &&
        overOdds !== null &&
        ...
        pushRow(makeRow({ event, selection: `${team} Over`, marketType: "team_corner_total", lineValue: overLine, oddsAmerican: overOdds, context }));
        pushRow(makeRow({ event, selection: `${team} Under`, marketType: "team_corner_total", lineValue: underLine, oddsAmerican: underOdds, context }));
        i += 6;
      }
    }
  }

...
export function parseBetSlip(text, sourceFileName = "", uploadBookmaker = "Auto") {
  ...
  if (sportsbook === "BetMGM") {
    return parseBetMgmSlip(cleaned, text, sourceFileName, sportsbook);
  }

  return parseDraftKingsLikeSlip(cleaned, text, sourceFileName, sportsbook);
}

function parseBetMgmSlip(cleaned, originalText, sourceFileName = "", sportsbook = "BetMGM") {
  const lines = cleaned
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);
  ...
  if (eventIndex !== -1) {
    ...
    const candidates = lines
      .slice(Math.max(0, (typeIndex !== -1 ? typeIndex : acceptedIndex !== -1 ? acceptedIndex : 0) + 1), eventIndex)
      .map(cleanTextLine)
...
function parseDraftKingsLikeSlip(cleaned, originalText, sourceFileName = "", forcedSportsbook = "") {
  const lines = cleaned
    .split("\n")
    .map((line) => line.trim())
    .filter(Boolean);

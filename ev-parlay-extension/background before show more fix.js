...
async function buildFanDuelOneNextNhlTabRawText() {
      ...
      if (currentNhlTab === "Points/Assists") {
        ...
        if (fanDuelTextShowsPlayerOddsAfterLabel("1+ Points", afterPoints)) {
      ...
      if (currentNhlTab === "Goalies") {
        ...
        showMoreClicked += await clickFanDuelShowMoreAggressively(6);
...
function isFanDuelUsefulCaptureText(text = rawPageText()) {
    ...
    async function clickFanDuelShowMoreAggressively(maxClicks = 10) {
      let total = 0;
      const clicked = new Set();

      for (let pass = 0; pass < 4 && total < maxClicks; pass += 1) {
        const candidates = Array.from(
          document.querySelectorAll("button, a, [role='button'], [tabindex], body *")
        )
          .filter((el) => {
            if (!isElementVisible(el)) return false;
        ...
        for (const el of candidates) {
          const rect = el.getBoundingClientRect();
          const key = `${Math.round(rect.top)}::${Math.round(rect.left)}::${Math.round(rect.width)}x${Math.round(rect.height)}`;
...
function fanDuelTextShowsOuRowsAfterLabel(label, text = rawPageText()) {
    ...
    async function clickFanDuelExactDrawerByLabelNow(label, options = {}) {
      ...
      if (options.skipIfOpen && fanDuelTextShowsOuRowsAfterLabel(label)) {
        return false;
      }
...
async function buildFanDuelOneNextNbaTabRawText() {
      ...
      if (ouHeader) {
        ...
        if (rowCount <= 0) {
          // Important: do NOT move to the next tab after a failed drawer open.
          // Queue the old scheduled-click fallback and let the next autopass retry this same tab.

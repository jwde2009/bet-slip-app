    ...
    function scheduleBetMgmDrawerClickByLabel(label) {
      ...
      window.setTimeout(() => {
        try {
          ...
          const buttons = Array.from(
            document.querySelectorAll("button.ds-accordion-header-clickable-area, button, [role='button'], body *")
          )
            .filter((candidate) => {
            ...
            .map((candidate) => findClickableAncestor(candidate) || candidate)

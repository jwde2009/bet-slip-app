...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  const missingMarketsForBook = summarizeMissingMarketsForToast(
    coverageSnapshot,
    tabSourceForSnapshot
  );
  ...
  if (missingMarketsForBook.length) {
    await safeShowToast(
      ...
      `Found ${missingMarketsForBook.length} missing ${tabSourceForSnapshot} market target${missingMarketsForBook.length === 1 ? "" : "s"}. Full targeted-click mode is the next patch.`,
      {
        loading: true,
        pulse: true,
      }
  ...
  const missingMarketSafeMode = shouldUseMissingMarketSafeMode({
    snapshot: coverageSnapshot,
    source: tabSourceForSnapshot,
  });

  let payload;

  if (missingMarketSafeMode.shouldUse) {
    await safeShowToast(
      sourceTabId,
      "Missing-market safe mode",
      "Captured current page only. Target-only click mode is not enabled yet, so this run will not open every drawer again.",
      {
        loading: true,
        pulse: true,
      }
    ...
    const singlePayload = await extractSinglePayloadFromTab(sourceTabId);
    payload = {
      source: singlePayload?.source || tabSourceForSnapshot || "",
      text: buildMissingMarketSafeModeText({
        source: singlePayload?.source || tabSourceForSnapshot || "",
        targets: missingMarketSafeMode.targets,
        currentText: singlePayload?.text || "",
      }),
      action: "",
      message: "",
    };
  } else {
    payload = isFanDuelTabUrl(tab.url)
...
async function buildFanDuelOneNextNhlTabRawText() {
      ...
      if (currentNhlTab === "Goalies") {
        ...
        showMoreClicked += await clickFanDuelShowMoreAggressively(12);
...
async function buildFanDuelOneNextNbaTabRawText() {
      ...
      if (ouHeader) {
        for (let attempt = 0; attempt < 3; attempt += 1) {
          ...
          if (fanDuelTextShowsOuRowsAfterLabel(ouHeader, beforeText)) {
            break;
          }
        ...
        const currentText = await waitForFanDuelUsefulCaptureText({ timeoutMs: 2500 });
        const rowCount = countFanDuelOuRowsAfterLabel(ouHeader, currentText);

        if (currentText && currentText.trim()) {
          captures.push(
            `FANDUEL_CURRENT_CAPTURE: ${currentTab || "unknown"}; direct O/U drawers ${openedLabels}; show more ${showMoreClicked}; O/U rows ${rowCount}\n${currentText}`
          );
        }
    ...
    function getBetMgmWorkflowSport(text = rawPageText()) {
      ...
      if (activeLooksNhl && !activeLooksNba) return "NHL";
  ...
  if (detectedSource === "FanDuel") {
    try {
      ...
      if (isFanDuelNbaGameLandingPage(fanDuelRaw)) {
        ...
        return {
          source: detectedSource,
          text: `FANDUEL_INITIAL_CAPTURE\n${fanDuelRaw}\n\nFANDUEL_SCHEDULED_NEXT_TAB: Popular -> Player Points`,
        };
      }
